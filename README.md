...
테스트 계정으로 바로 로그인하실 수 있습니다.

| 이메일 | 비밀번호 |
|---|---|
| `tester@gmail.com` | `testtest` |

- 직접 계정을 만들려면 `회원가입하기`에서 이메일과 비밀번호(6자 이상)로 가입할 수 있습니다. 이메일 인증 절차는 없습니다.
- `Google로 계속` 버튼으로 구글 계정 로그인도 가능합니다.

### 데모 그룹

테스트 계정으로 로그인하면 사진과 메모가 쌓인 데모 그룹이 목록에 바로 나타납니다.
새 그룹을 만들어 보시려면 그룹 목록 우상단 **+** → **그룹방 만들기**를 이용하세요.

### 테스트 시나리오

1. **기억 남기기** — 하단 바 `+` → 사진 선택 → 날짜·제목·이야기 입력 → 저장
2. **그룹 피드** — 올라온 기억에 좋아요와 댓글을 남깁니다
3. **AI 추억 찾기** — 하단 바 왼쪽 별 버튼 → "강릉 여행", "겨울에 다 같이 먹은 거" 처럼 물어봅니다
   - 조건이 막연하면 AI가 한 번 되묻고, 충분하면 그룹 기록 안에서만 검색해 기억 카드를 함께 보여줍니다
   - 검색 결과에 없는 기억은 만들어내지 않습니다
4. **결과 카드 탭** → 기억 상세로 이동
5. **채팅방 공유** — 상세 화면의 보내기 아이콘으로 그룹 채팅에 공유
